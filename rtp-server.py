...
def packetize(msg):
    num = int(
        len(msg) / MAX_PAYLOAD_LENGTH + (1 if len(msg) % MAX_PAYLOAD_LENGTH != 0 else 0)
    )
    packets = []
    for x in range(num - 1):
        payload = msg[
            (x * MAX_PAYLOAD_LENGTH) : ((x * MAX_PAYLOAD_LENGTH) + MAX_PAYLOAD_LENGTH)
        ]
    ...
    while True:
        ...
        while last_type == DATA:
            ...
            if r_type == TERM:
                ...
                print(
                    "Server received invalid packet type while receiving message! \
                        Exiting..."
                )
                exit(1)
            ...
            if random() < DROP_RATE:
                print(
                    "SIMULATING NETWORK CORRUPTION - ERROR IN CHECKSUM OCCURRED...",
                    end=" ",
                )
                r_chksum += 13
        ...
        while i < len(packets):
            ...
            if random() < DROP_RATE:
                print(
                    "SIMULATING NETWORK CORRUPTION - CREATING ERROR IN CHECKSUM...",
                    end=" ",
                )
                curr_pkt[1] += 13
            ...
            if r_type == TERM:
                ...
                print(
                    "Server received invalid packet type while sending message! \
                        Exiting..."
                )
                exit(1)
...
parser.add_argument(
    "-p",
    "--port",
    help="\t\tInteger port that the server should listen on",
    type=int,
    required=True,
)
parser.add_argument(
    "-c",
    "--corruption",
    help="\t\tFloat that determines corruption percentage. \n\
        (e.g. -c .5 will give the connection a 50% corruption rate)",
    type=float,
    default=0.0,
)
